...
${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/python/bin/pip install requests
cd $OPENSHIFT_TMP_DIR
 
cat <<'EOF'  >> youtube_download.py
import sys
...
nohup sh -c " ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/python/bin/python youtube_download.py"> $OPENSHIFT_LOG_DIR/youtube_download.log /dev/null 2>&1 &  
tail -f  $OPENSHIFT_LOG_DIR/youtube_download.log
