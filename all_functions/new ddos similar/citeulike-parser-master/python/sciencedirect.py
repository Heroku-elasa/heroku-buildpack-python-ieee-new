...
if __name__ == "__main__":
    ...
    urllib2.ProxyHandler({"http": "http://143.89.225.246:3128"})
    s = 'q'

    # url = sys.stdin.readline().strip();
    url = 'http://www.sciencedirect.com/science/article/pii/S105905601400029X'
    # handle2(url)
    cUrl = canon_url(url)
    page = urlopen(cUrl).read()
...
print page
# m = re.search(r'<a(?: id="[^"]+")? href="http://dx.doi.org/([^"]+)"', page)
m = re.search(r'<a(?: id="[^"]+")? SDM.pm.doi ([^"]+)"', page)
...
# doi = m.group(1)
print re.search(r'^10[.](1016|1006|1053)/',doi)
abstract = scrape_abstract(page)
#	try:
#		abstract = scrape_abstract(page)
#	except:
...
try:
        ...
        print "\t".join(["status", "error",
                         "There was an internal error processing this request. Please report this to bugs@citeulike.org quoting error code %d." % line])
        raise
